...
**Version 3.8.0b0** | Python Dependency Vulnerability Scanner
...
| `--debug [1\|true]` | Enable debug logging | — |
...
| `--save-as <FORMAT:PATH>` | Save report to file (e.g., `json:report.json`) | — |
...
| `--jsonl-path <PATH>` | Directory for JSONL output files | — |
...
| `-asul` / `--auto-security-updates-limit <TYPES>` | Limit update types: `patch`, `minor`, `major` | — |
| `--save-json <PATH>` | Save JSON report to file | — |
| `--save-html <PATH>` | Save HTML report to file | — |
| `--audit-and-monitor` | Send results to Safety Platform | `false` |
...
```bash
safety codebase init [OPTIONS] [PATH]
```

| Option | Description |
|--------|-------------|
| `--name <NAME>` | Project/codebase name |
| `--link-to <PROJECT_ID>` | Link to existing Safety Platform project |
| `--disable-firewall` | Skip firewall setup |
...
Writes settings to `~/.safety/config.ini`.
...
```bash
safety generate policy-file [--path <OUTPUT_PATH>]
```
...
```bash
safety validate policy-file [--path <POLICY_FILE_PATH>]
```
...
safety check -r requirements.txt --output json --json-version 0.5
```
...
Global Safety configuration. Located at `~/.safety/config.ini` (user-level) or `/etc/.safety/config.ini` (system-level). System-level takes precedence if it exists.
...
| Variable | Purpose |
|----------|---------|
| `SAFETY_GIT_COMMIT` | Git commit SHA for telemetry |
| `SAFETY_OS_DESCRIPTION` | OS description for telemetry |
| `SAFETY_OS_RELEASE` | OS release for telemetry |
...
```bash
safety generate policy-file --path .safety-policy.yml
```
...
```bash
safety validate policy-file --path .safety-policy.yml
```
...
| `safety generate policy-file` | Active | No | Create policy template |
| `safety validate policy-file` | Active | No | Validate policy file |
